...
# ---------- TELEGRAM BOT ----------
bot = TelegramClient("bot2_session", API_ID, API_HASH).start(bot_token=BOT_TOKEN)
user_states = {}
active_otp_clients = {}
...
async def process_redeem_code(user_id, code):
    ...
    # find an available account with matching category
...
""",
    ...
    # start OTP forwarding for the buyer
...
async def message_handler(event):
    ...
    if user_id in ADMIN_IDS:
        ...
        if state == 'AWAITING_PHONE':
            ...
            try:
                price = float(text)
                # store temporarily and ask for category
                user_states[user_id] = {
